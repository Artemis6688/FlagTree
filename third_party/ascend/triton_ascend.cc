...
void init_triton_ascend_ir(py::module &&m) {
  ...
  builder_cls
      ...
      .def("create_partition_view",
           [](TritonOpBuilder &self, Value baseView, std::vector<int64_t> tile,
              const std::string &padding) -> Value {
             auto kind =
                 llvm::StringSwitch<triton::tv::PadKind>(padding)
                     .Case("zero", triton::tv::PadKind::Zero)
                     .Case("nan", triton::tv::PadKind::Nan)
                     .Case("inf", triton::tv::PadKind::Inf)
                     .Case("-inf", triton::tv::PadKind::NInf)
                     .Default(triton::tv::PadKind::Zero);
             return triton::tv::createPartitionView(
                 self.getBuilder(), self.getLastLoc(), baseView, tile, kind);
           })
