...
# dst_space/src_space argument. This exercises the DSA direct-lowering path
# (tile.load/tile.store -> TileIRToHIVM -> hivm.load/hivm.store), not the
# tl.load/tl.store -> tv.view_load/store path.
#
# NOTE: cases are validation only. This stage lowers the GM<->UB DMA; other
# on-chip spaces are reserved (explicit API, not yet lowered).
