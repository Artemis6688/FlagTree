...
# ---- Vector sub-tile split --------------------------------------------------
# The Vector scope processes BLOCK_M×BLOCK_N softmax tiles sequentially in
# SUB_VEC_NUM sub-tiles, each SUB_M×BLOCK_N. This halves peak UB pressure:
# each sub-tile iteration only materializes SUB_M-sized buffers, allowing
# PlanMemory to fit BLOCK_M=256, BLOCK_N=128 within 192KB UB after optimization.
# The Cube side is untouched: MM1/MM2 still produce/consume full BLOCK_M tiles.
...
def _vec1_softmax(
    ...
    M-dimension sub-tiling: processes BLOCK_M rows in SUB_VEC_NUM sequential
    sub-tiles of SUB_M rows each. Each sub-tile maintains independent online
    softmax state (neg_max_even/odd) across CB iterations. This halves peak UB
    usage from ~448KB to ~224KB, which PlanMemory optimizes to ≤192KB.

    Returns updated (neg_max_even_0, neg_max_odd_0, neg_max_even_1, neg_max_odd_1).
    """
    ...
    if idx_in_conbine == 0:
        neg_max_even_0 = tl.full((SUB_M, ), 2**30, tl.float32)
        neg_max_odd_0 = tl.full((SUB_M, ), 2**30, tl.float32)
        neg_max_even_1 = tl.full((SUB_M, ), 2**30, tl.float32)
        neg_max_odd_1 = tl.full((SUB_M, ), 2**30, tl.float32)
    ...
    for cb_idx in range(CB):
        ...
        for sub_idx in tl.static_range(SUB_VEC_NUM):
            ...
            # load score[sub_idx*SUB_M : (sub_idx+1)*SUB_M, :] from workspace_s GM -> UB (MTE2)
            score_load_bp = tl.make_block_ptr(
                workspace_s + cid * RING * CB * BLOCK_M * BLOCK_N + ring_slot * CB * BLOCK_M * BLOCK_N +
                cb_idx * BLOCK_M * BLOCK_N + offset_m * BLOCK_N, (SUB_M, BLOCK_N), (BLOCK_N, 1), (0, 0),
                (SUB_M, BLOCK_N), (1, 0))
            attn_score_block = tl.load(score_load_bp).to(tl.float32)

            # Select state for this sub-tile
            if sub_idx == 0:
            ...
            if IS_CAUSAL:
                ...
                # row ids absolute in the sequence, offset by this sub-tile's slice
                q_row_idx = q_global_head_idx * BLOCK_M + offset_m + tl.arange(0, SUB_M)
            ...
            # layout: [NUM_CORES, RING, CB, BLOCK_M] — each sub-tile writes its own SUB_M rows
            rescale_offset = (cid * RING * CB * BLOCK_M + ring_slot * CB * BLOCK_M + cb_idx * BLOCK_M + offset_m)
            rescale_store_bp = tl.make_block_ptr(workspace_rescale + rescale_offset, (SUB_M, 1), (1, 1), (0, 0),
                                                 (SUB_M, 1), (1, 0))
            tl.store(rescale_store_bp, rescale[:, None])
            expsum_store_bp = tl.make_block_ptr(workspace_expsum + rescale_offset, (SUB_M, 1), (1, 1), (0, 0),
                                                (SUB_M, 1), (1, 0))
            tl.store(expsum_store_bp, block_expsum[:, None])

            # update running max ping-pong for this sub-tile
            if cur_parity == 0:
                neg_max_even = neg_max_new
            else:
                neg_max_odd = neg_max_new

            # softmax_p -> workspace_p GM (MTE3): this sub-tile's SUB_M rows
            ...
            # Save back state for this sub-tile
            if sub_idx == 0:
    ...
    return neg_max_even_0, neg_max_odd_0, neg_max_even_1, neg_max_odd_1
...
def _vec2_accumulate(
    ...
    M-dimension sub-tiling: processes BLOCK_M rows in SUB_VEC_NUM sequential
    sub-tiles of SUB_M rows each. Each sub-tile maintains independent accumulator
    state (acc_o, softmax_denom) across CB iterations. Final output is written
    per sub-tile.

    Returns updated (acc_o_0, softmax_denom_0, acc_o_1, softmax_denom_1).
    """
    ...
    for cb_idx in range(CB):
        ...
        for sub_idx in tl.static_range(SUB_VEC_NUM):
            ...
            # load pv_acc[sub_idx*SUB_M : (sub_idx+1)*SUB_M, :] from workspace_pv (MTE2)
            pv_load_bp = tl.make_block_ptr(
                workspace_pv + cid * RING * CB * BLOCK_M * DIM + prev_ring_slot * CB * BLOCK_M * DIM +
                cb_idx * BLOCK_M * DIM + offset_m * DIM, (SUB_M, DIM), (DIM, 1), (0, 0), (SUB_M, DIM), (1, 0))
            pv_acc = tl.load(pv_load_bp).to(tl.float32)

            # load rescale and block_expsum written by Vec1 for this sub-tile's SUB_M slice
            prev_rescale_offset = (cid * RING * CB * BLOCK_M + prev_ring_slot * CB * BLOCK_M + cb_idx * BLOCK_M +
                                   offset_m)
            rescale_load_bp = tl.make_block_ptr(workspace_rescale + prev_rescale_offset, (SUB_M, 1), (1, 1), (0, 0),
                                                (SUB_M, 1), (1, 0))
            rescale = tl.reshape(tl.load(rescale_load_bp).to(tl.float32), (SUB_M, ))
            expsum_load_bp = tl.make_block_ptr(workspace_expsum + prev_rescale_offset, (SUB_M, 1), (1, 1), (0, 0),
                                               (SUB_M, 1), (1, 0))
            block_expsum = tl.reshape(tl.load(expsum_load_bp).to(tl.float32), (SUB_M, ))

            # Select state for this sub-tile
            if sub_idx == 0:
                acc_o = acc_o_0
                softmax_denom = softmax_denom_0
            else:
                acc_o = acc_o_1
                softmax_denom = softmax_denom_1
            ...
            if prev_kv_idx == 0:
                ...
                rescale_bc = tl.broadcast_to(rescale[:, None], (SUB_M, DIM))
                acc_o = acc_o * rescale_bc + pv_acc
                softmax_denom = softmax_denom * rescale + block_expsum

            if prev_kv_idx == NUM_KV_BLOCKS - 1:
                # last KV block: divide by softmax denominator and write output rows
                denom_bc = tl.broadcast_to(softmax_denom[:, None], (SUB_M, DIM))
                ...
                o_bp = tl.make_block_ptr(Out + prev_batch_idx * sOb + prev_head_idx * sOh, (S, DIM), (sOs, sOd),
                                         ((prev_global_head_idx * BLOCK_M + offset_m).to(tl.int32), 0), (SUB_M, DIM),
                                         (1, 0))
            ...
            if sub_idx == 0:
                acc_o_0 = acc_o
                softmax_denom_0 = softmax_denom
            else:
                acc_o_1 = acc_o
                softmax_denom_1 = softmax_denom
    ...
    return acc_o_0, softmax_denom_0, acc_o_1, softmax_denom_1
...
def flash_attention_fwd_3task_kernel(
    ...
    BLOCK_N: tl.constexpr,
    DIM: tl.constexpr,
    SUB_M: tl.constexpr,  # = BLOCK_M // SUB_VEC_NUM: rows owned by one Vector sub-core
):
    cid = tl.program_id(0)
    ...
    # -- Vector side (UB registers): online-softmax state for 2 sub-tiles -------
    # Each sub-tile maintains independent state across CB iterations. Since we
    # process sub-tiles sequentially (not parallel sub-cores), we need 2 sets of
    # state: one per sub-tile. Each set holds SUB_M rows of accumulator state.
    acc_o_0 = tl.zeros((SUB_M, DIM), tl.float32)
    acc_o_1 = tl.zeros((SUB_M, DIM), tl.float32)
    softmax_denom_0 = tl.zeros((SUB_M, ), tl.float32)
    softmax_denom_1 = tl.zeros((SUB_M, ), tl.float32)
    ...
    for g in range(num_global_tasks + 1):
        ...
        with tle.scope(core_mode="vector"):
            ...
            if g < num_global_tasks:
                idx_in_conbine = g % conbined_block_num
                ring_slot = g % RING
                neg_max_even_0, neg_max_odd_0, neg_max_even_1, neg_max_odd_1 = _vec1_softmax(
                    workspace_s,
                    workspace_p,
                    workspace_rescale,
                    workspace_expsum,
            ...
            if g >= 1:
                ...
                acc_o_0, softmax_denom_0, acc_o_1, softmax_denom_1 = _vec2_accumulate(
                    Out,
                    workspace_pv,
                    workspace_rescale,
                    workspace_expsum,
...
def flash_attention_fwd(q, k, v, combine_batch, is_causal=False):
    ...
    # the Vector scope splits each BLOCK_M tile between two sub-cores, so the
    # M dimension must divide evenly into full tiles (no partial tail row).
    assert S % BLOCK_M == 0, f"S ({S}) must be divisible by BLOCK_M ({BLOCK_M})"
    assert BLOCK_M % SUB_VEC_NUM.value == 0, f"BLOCK_M ({BLOCK_M}) must be divisible by {SUB_VEC_NUM.value} sub-cores"
