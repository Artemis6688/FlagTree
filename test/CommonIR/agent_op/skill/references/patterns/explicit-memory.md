...
Allocate on-chip buffers explicitly with `tle.dsa.alloc(shape, dtype, mem_addr_space)` into UB / L1, and use GM workspace tensors (kernel parameters) for anything that crosses cores. Convert buffers to register tensors with `to_tensor` for compute, and slice without copying via `subview`.
...
Verified dot-operand forms: whole-tile `tile_copy` GM→L1, then `tl.dot(tile_to_tensor(l1_buf, writable=False), ...)`; or plain register tensors from `tl.load` / `tl.make_block_ptr` (the MM path in 3-task pipeline kernels).
...
1. **Identical-shape `tile_alloc`s are merged.** `commonir_to_hivm` merges allocs of identical shape/dtype into one physical cbuf (two same-shape ping-pong buffers silently alias → WRONG results). For multiplicity, allocate ONE buffer with a doubled leading dim and ping-pong via `subview`.
...
5. **Dynamic leading-dim `dsa.copy`** (e.g. `copy(src, buf, [tail, DIM])` with dynamic first extent) appeared in a failing kernel; the verified form uses constexpr extents `[tl.constexpr(BM), tl.constexpr(D)]`. Dynamic tails are verified for 1D/2D UB copies with small shapes (`[4, tail_d]`) — the failing case combined it with L1 + dot.
...
- Capacity: explicit UB total × (2 if `multibuffer=True`) ≤ ~192 KB per core.
...
- Run the sync linter if the kernel also uses manual sync.
