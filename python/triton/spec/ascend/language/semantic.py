...
# ===----------------------------------------------------------------------===
    ...
    def _tensor_view_index_and_encoding(self, view: "tl.tensor_view", index, tile, traversal_strides, sparse_dim):
        ...
        if tile is None:
            enc_tile = self.builder.get_tensor_view_encoding_tile(view.handle)
            assert len(enc_tile) > 0, \
                "`tile` is required when the tensor_view has no encoding (base view); " \
                "use `make_partition_view` to attach one, or pass `tile` explicitly"
            tile = list(enc_tile)
        else:
    ...
    def make_partition_view(self, base: TensorTy, shape, strides, tile: list,
                            padding_value: str = "zero") -> "tl.tensor_view":
        # Build the base logical view, then attach a partition encoding that
        # records the tile shape and out-of-bounds padding. tl.load/tl.store
        # only need an `index` after this: the tile comes from the encoding.
        view = self.make_tensor_view(base, shape, strides)
        assert all(isinstance(t, int) for t in tile), "tile must be constant integers"
        assert len(tile) == view.rank, f"tile rank ({len(tile)}) != view rank ({view.rank})"
        valid_padding = ("zero", "nan", "inf", "-inf")
        assert padding_value in valid_padding, \
            f"`padding_value` must be one of {valid_padding}, but got {padding_value!r}"
        handle = self.builder.create_partition_view(view.handle, tile, padding_value)
        return tl.tensor_view(handle, view.dtype, view.rank)
