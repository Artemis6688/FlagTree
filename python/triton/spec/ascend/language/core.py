...
def load(pointer, mask=None, other=None, boundary_check=(), padding_option="", cache_modifier="", eviction_policy="",
    ...
    Return a tensor of data whose values are loaded from memory at location defined by `pointer`:
        ...
        (4) If `pointer` is a `tl.tensor_view`, `index` selects the loaded tile.
            `tile` is extracted from the view's encoding (set by `make_partition_view`),
            or can be passed explicitly. `traversal_strides` and `sparse_dim` are optional.
            Masks apply only to sparse dimensions; regular boundaries come from the view shape.
    ...
    :param tile: (`tl.tensor_view` only) the tile's shape, one entry per dimension; extracted from view encoding if not provided
    :type tile: sequence of `tl.constexpr` or int, optional with `pointer` a `tl.tensor_view` (uses view encoding)
    ...
    if isinstance(pointer, tensor_view):
        ...
        assert index is not None, "`index` is required when loading a `tl.tensor_view`"
...
def store(pointer, value, mask=None, boundary_check=(), cache_modifier="", eviction_policy="", index=None, tile=None,
    ...
    Store a tensor of data into memory locations defined by `pointer`.
        ...
        (4) If `pointer` is a `tl.tensor_view`, `index` selects the stored tile.
            `tile` is extracted from the view's encoding (set by `make_partition_view`),
            or can be passed explicitly. `traversal_strides` and `sparse_dim` are optional.
            Masks apply only to sparse dimensions.
    ...
    if isinstance(pointer, tensor_view):
        ...
        assert index is not None, "`index` is required when storing to a `tl.tensor_view`"
...
def make_partition_view(base: tensor, shape, strides, tile, padding_value="zero", _semantic=None) -> tensor_view:
    """
    Reinterpret a scalar pointer as an N-dimensional logical tensor partitioned
    into tiles of shape ``tile``. The returned ``tensor_view`` carries the tile
    metadata (and out-of-bounds padding) in the IR, so subsequent ``tl.load`` /
    ``tl.store`` calls only need an ``index`` argument to select a tile.

    :param base: The base pointer to the parent tensor
    :param shape: The shape of the parent tensor, one entry per dimension
    :param strides: The element strides of the parent tensor, one entry per
        dimension
    :param tile: The tile shape, one entry per dimension (constexpr ints)
    :param padding_value: Value written into the out-of-bounds (tail) lanes of a
        boundary tile. One of ``"zero"`` (default), ``"nan"``, ``"inf"`` or
        ``"-inf"``.
    """
    tile = [_unwrap_if_constexpr(t) for t in _unwrap_if_constexpr(tile)]
    padding_value = _unwrap_if_constexpr(padding_value)
